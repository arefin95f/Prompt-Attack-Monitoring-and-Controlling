...
const ACC_MODE_HELP = {
    heldout: 'Scores the full defense on the full held-out test set.',
...
function syncAccModeUI() {
    ...
    if ($('acc-run-hint')) {
        $('acc-run-hint').textContent = mode === 'ablation'
            ? 'Full test set × each ablation — this can take a long time.'
            : 'Uses the full held-out test set. Stay on this page.';
    }
}
...
function renderAccuracyReport(report, meta = {}) {
    ...
    if (report.aggregate) {
        ...
        body += `<p class="acc-help">Pattern-bank averages across rounds (appendix).</p>`;
